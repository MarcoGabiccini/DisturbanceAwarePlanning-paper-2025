...
%The kinematic tree of the two models is slightly different, but the procedure used to build the dynamic model is similar apart from a non-trivial aspect that will be discussed later on. In the remainder of this study, unless otherwise specified, the descriptions provided apply to both models. The figures related to Sections~\ref{sec:suspanal}-\ref{sec:tyremodel} are courtesy of the authors.
