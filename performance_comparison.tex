...
\begin{figure}[t]
	...
	\caption{Open-Loop. Axle saturation $S_j(\bx,\bu)$ for front axle (j=1, first panel) and rear axle (j=2, second panel) with respect to the curvilinear abscissa $\al$. Nominal configuration in dashed black, TLC in green, FLC in red, TLC+FLC in blue.
...
\begin{figure}[b!]
	...
	\caption{Closed-Loop. Axle saturation $S_j(\bx,\bu)$ for front axle (j=1, first panel) and rear axle (j=2, second panel) with respect to the curvilinear abscissa $\al$. Nominal configuration in dashed black, TLC in green, FLC in red, TLC+FLC in blue.
		%The dashed black line corresponds to the reference (nominal) configuration, while the green, red, and blue lines represent the robustified configurations TLC, FLC, and TLC+FLC, respectively.
	}
...
The same key signals displayed for the previous case are shown in Figure~\ref{fig:cl_telemetries}, while the axle saturation ratios $S_j$ are illustrated in Figure~\ref{fig:cl_saturation}.
...
To provide an empirical validation of the closed-loop robustified planning we simulated the evolution of the controlled vehicle with random initial conditions and included noise realizations. The random values for the initial state and the noise follow Gaussian distributions with covariance matrices $\bP_0$ and $\bQ$, respectively, as introduced in~\eqref{eq:dP}. This analysis cannot be performed for the open-loop approach, as it does not compute a stabilizing controller---resulting in trajectories that would significantly deviate from the track limits beyond the prediction horizon.
