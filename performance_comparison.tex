...
The entire circuit is parametrized by the curve parameter $\alpha \in [0,1]$, and the selected sector corresponds to the interval $\left[0.70, 0.77\right]$. Checkpoints are indicated by labels and are uniformly spaced by $\De\al=0.01$. This sector includes two distinct corners: a low-speed turn from $\al = 0.72$ to $\al = 0.73$, and a high-speed turn from $\al = 0.75$ to $\al = 0.76$, allowing for the evaluation of vehicle behavior across different dynamic scenarios. 

The chosen sector has been discretized with 140 intervals, corresponding to one grid point each 0.43\,m. 
The optimization problems are formulate within the CasADi+MATLAB framework~\cite{Andersson:MPC:2019}, and solved using the interior point method implemented in IPOPT~\cite{Wachter:MP:2006}. 
...
Observing the left panel of Figure~\ref{fig:ol_sensitivities}, it is evident that the trajectories corresponding to configurations with lower values of $\ga^\textrm{TLC}$ yield smaller lateral margins with respect to the track boundaries. In contrast, the trajectory associated with $\ga^\textrm{TLC} = 3$\,--- represented by the light green line \,--- remains significantly farther from the edges.
...
% Calculus times 
We provide here some details on the software implementation and computation times to complement the comparison between the two proposed methods. All optimizations were performed on a machine equipped with an Intel Core i9-13980HX processor and 32\,Gb of RAM. Only the solver time is reported for each configuration, as the problem build time is negligible.
The nominal problem involves 4200 optimization variables and is solved in 7.5\,s. The open-loop configuration, using a prediction horizon of $H=4$ as adopted in this study, includes 48300 variables and requires 128\,s to solve. The closed-loop method first solves the nominal problem, then performs the Step~2 in 0.5\,s, and finally formulates an additional problem with 14700 variables, which is solved in 33\,s, resulting in a total computation time of 40\,s.
